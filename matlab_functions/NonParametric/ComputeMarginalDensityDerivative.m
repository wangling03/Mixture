function [g]=ComputeMarginalDensityDerivative(x,y,p_tilde)

%--------------------------------------------------------------------------
% This function computes the weighted kernel estimate of the 
% marginal density derivative of x at y 
...
q1=(1/(sqrt(2*pi)*N*h*h*h))*(1-p_tilde);

q2=(1/(sqrt(2*pi)*N*h*h*h))*((1-p_tilde).*x);
...
[kde1]=figtreeEvaluateIfgt(1,N,M,1,x_shifted_scaled,sqrt(2)*h*scale,q1,y_shifted_scaled,p_max,K,clusterIndex,clusterCenter,clusterRadii,r,epsil);
[kde2]=figtreeEvaluateIfgt(1,N,M,1,x_shifted_scaled,sqrt(2)*h*scale,q2,y_shifted_scaled,p_max,K,clusterIndex,clusterCenter,clusterRadii,r,epsil);
